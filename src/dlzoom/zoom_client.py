...
class ZoomClient:
            ...
            try:
                ...
                if response.status_code in (429, 500, 502, 503, 504):
                    if attempt < retry_count - 1:
                        ...
                        status_name = "Rate limit" if response.status_code == 429 else "Server error"
                        logging.warning(
                        ...
                        if response.status_code == 429:
                            raise RateLimitedError(
                                "Rate limit exceeded",
                                details="Too many requests to Zoom API. Please retry after some time."
                            )
                        else:
                            raise ZoomAPIError(
                                f"Zoom API server error (HTTP {response.status_code})",
                                details=f"Server returned {response.status_code} after {retry_count} retries"
                            )
                ...
                if status_code == 401:
                    ...
                    raise PermissionDeniedError(
                        "Permission denied",
                        details="Check OAuth scopes for your Zoom app"
                    )
    ...
    def get_all_participants(self, uuid: str) -> List[Dict[str, Any]]:
        ...
        while True:
            response = self.get_past_meeting_participants(uuid, page_size=300, next_page_token=next_token)
